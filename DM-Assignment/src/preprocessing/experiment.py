...
def compare_preprocessing_pipelines():
    ...
    print(
        f"   Total combinations: {len(imputation_strategies) * len(outlier_strategies)}"
    )
    ...
    results = []
    total_combinations = len(imputation_strategies) * len(outlier_strategies)
    ...
    for impute_name, imputer_factory in imputation_strategies.items():
        for outlier_name, outlier_detector_factory in outlier_strategies.items():
            ...
            pipeline_name = f"{impute_name}+{outlier_name}"
            ...
            for fold, (train_idx, val_idx) in enumerate(cv.split(X, y), 1):
                ...
                if outlier_detector is not None:
                    ...
                    X_train_filled = X_train_filled[inlier_mask].reset_index(drop=True)
